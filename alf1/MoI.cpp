...
#include<C:\Users\windows7\projects\NS_Integrator\alf1\interpol.h>
...
dydx[0]= (-G*(alf1.rho(log10(y[0]))*c2 + y[0])*(y[1]*c2 + 4.0*M_PI*pow(x,3)*y[0]))/(pow(c,4)*x*(x - (2.0*G*y[1]/c2)));
dydx[1]= 4.0*M_PI*pow(x,2)*alf1.rho(log10(y[0]));
dydx[2]= 8.0*M_PI*pow(x,4)*alf1.rho(log10(y[0]))/3.0;
...
{    int n = 47;
    ...
    Doub pc = alf1.Pres(j); // central pressure
    const Int nvar=3;
...
ofstream file3("./alf1/moi_alf1.dat");
for(int i = 0; i < n; i++)
