...
dydx[0]= -G*y[1]*rho(y[0])/pow(x,2);     // (-G*(rho(y[0])*pow(c,2) + y[0])*(y[1]*pow(c,2) + 4.0*M_PI*pow(x,3)*y[0]))/(pow(c,4)*x*(x - (2.0*G*y[1]/pow(c,2))));
...
//for (Int i=0;i<out.count;i++)
     ...
     cout<<out.xsave[out.count -1]<< " "<<ystart[1]<<" "<<ystart[0]<<endl;
     j = j + 0.01;
     
}

ofstream file2("./ND/massradius_newton.dat");
for(int i = 0; i < 320; i++)
    file2 << radius[i] << " " << mass[i] <<endl;
file2.close();
